...
Key components:
  * WDW (Wavelet Disjoint Weights) - kernel weights stored in wavelet domain.
    Four oriented subbands (LL, LH, HL, HH) provide spatially-localized bases
    that the global FFT basis cannot capture.
...
Reference: FDConv - Frequency Dynamic Convolution for Dense Image Prediction
...
class WDConv(nn.Conv2d):
    """Wavelet Dynamic Convolution. Drop-in replacement for nn.Conv2d."""
    ...
    def __init__(self,
        ...
        self.alpha = (min(self.out_channels, self.in_channels) // 2
                      * self.kernel_num * self.param_ratio / param_reduction)
    ...
    def convert2dwtweight(self, convert_param):
        ...
        dwt_weight = haar_dwt2d_pack(weight)

        indices_all = get_wavelet_indices(H, W)
        ...
        if self.param_reduction < 1:
            ...
            dwt_weight_param = flat[None].repeat(self.param_ratio, 1) / scale
        else:
            dwt_weight_param = dwt_weight[None].repeat(self.param_ratio, 1, 1) / scale
    ...
    def get_WDW(self):
        ...
        scale = max(min(self.out_channels, self.in_channels) // 2, 1)
        return dwt[None].repeat(self.param_ratio, 1, 1) / scale
    ...
    def forward(self, x):
        ...
        DWT_map = torch.zeros((batch_size, H, W), device=x.device, dtype=x.dtype)
        kernel_att = kernel_att.reshape(batch_size, self.param_ratio, self.kernel_num, -1)

        dwt_w = self.dwt_weight if hasattr(self, 'dwt_weight') else self.get_WDW()

        for i in range(self.param_ratio):
            indices = self.indices[i]
            if self.param_reduction < 1:
                w = dwt_w[i].reshape(self.kernel_num, -1)[None]
                DWT_map[:, indices[0, :, :], indices[1, :, :]] += w * kernel_att[:, i]
            else:
                w = dwt_w[i][indices[0, :, :], indices[1, :, :]][None] * self.alpha
                DWT_map[:, indices[0, :, :], indices[1, :, :]] += w * kernel_att[:, i]

        # Inverse wavelet -> spatial kernel weights
